 ...
 */
export const currencyFormatting = (currency: string, options: { enforceSupportedCurrencies?: boolean } = {}) => {
  const { enforceSupportedCurrencies = true } = options || {};
  if (enforceSupportedCurrencies && !subUnitDivisors[currency as keyof typeof subUnitDivisors]) {
  ...
  return subUnitDivisors[currency as keyof typeof subUnitDivisors] === 1
    ? {
        style: 'currency',
        currency,
