...
import { CategoryNode, DefaultFilter, ListingFields } from '@appTypes/config';
 ...
 */
export const parseSelectFilterOptions = (uriComponentValue: string) => {
 ...
 */
export const constructQueryParamName = (key: string, scope: Scope) => {
  const prefixedKey =
    scope === 'meta' || scope === 'metadata' ? `meta_${key}` : `pub_${key}`;
...
export const getQueryParamNames = (
  ...
  defaultFiltersConfig: DefaultFilter[],
) => {
  const queryParamKeysOfDefaultFilters = defaultFiltersConfig.reduce(
    (pickedKeys, config) => {
      const { key, schemaType, scope } = config;
      const newKeys =
        schemaType === 'category' &&
        'nestedParams' in config &&
        config.nestedParams
          ? config.nestedParams?.map(p => constructQueryParamName(p, scope))
          : schemaType === 'listingType'
          ? [constructQueryParamName(key, scope)]
          : [key];
      return [...pickedKeys, ...newKeys].filter(t => t !== undefined);
    },
    [] as string[],
  );
...
export const isAnyFilterActive = (
  ...
  const getQueryParamKeysOfGivenFilters = (
    pickedKeys: string[],
    key: string,
  ) => {
...
interface SelectTreeOption {
  option: string;
  label: string;
  suboptions?: SelectTreeOption[];
}

export const convertCategoriesToSelectTreeOptions = (
  categories: CategoryNode[],
) => {
  const convertSubcategoryData = (params: CategoryNode): SelectTreeOption => {
 ...
 */
export const isMainSearchTypeKeywords = (config: AppConfig) =>
  config.search?.mainSearch?.searchType === 'keywords';

/**
 * Check if the origin parameter is currently active.
 */
export const isOriginInUse = (config: AppConfig) =>
 ...
 */
export const isStockInUse = (config: AppConfig) => {
