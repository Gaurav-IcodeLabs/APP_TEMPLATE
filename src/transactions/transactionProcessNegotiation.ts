...
const filterOfferTransitions = (transitions: Transition[], offerTransitions: string[]) => {
 ...
 * @returns {boolean}
 */
export const isValidNegotiationOffersArray = (transitions: Transition[], offers: Transition[]) => {
 ...
 * @returns {boolean}
 */
export const isNegotiationState = (state: string) => {
  if (state == null) {
    return false;
  }
 ...
 */
export const getTransitionsWithMatchingOffers = (transitions: Transition[], offers: Transition[]) => {
...
export const isRelevantPastTransition = (transition: string) => {
  return [
...
export const isCustomerReview = (transition: string) => {
  return [transitions.REVIEW_1_BY_CUSTOMER, transitions.REVIEW_2_BY_CUSTOMER].includes(transition);
};

// Processes might be different on how reviews are handled.
// Default processes use two-sided diamond shape, where either party can make the review first
export const isProviderReview = (transition: string) => {
...
// enough.
export const isPrivileged = (transition: string) => {
  return [
    transitions.MAKE_OFFER,
    transitions.MAKE_OFFER_AFTER_INQUIRY,
    transitions.MAKE_OFFER_FROM_REQUEST,
...
export const isCompleted = (transition: string) => {
...
export const isRefunded = (transition: string) => {
